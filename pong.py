...
class Pong:
    ...
    def initialize_players(self):
        if (self.player_nmbr == 0):
            ...
            pygame.Rect(self.screen_width - self.players_width - 5, (self.screen_height - self.players_height) / 2,
                        self.players_width, self.players_height))
        else:
            return (
            pygame.Rect(self.screen_width - self.players_width - 5, (self.screen_height - self.players_height) / 2,
                        self.players_width, self.players_height),
    ...
    def update_ball(self):
        ...
        if self.ball.colliderect(self.player) or self.ball.colliderect(
                self.enemy):  # Might add ball_speed_y=0 when hit near center
            self.ball_speed_x *= -1
