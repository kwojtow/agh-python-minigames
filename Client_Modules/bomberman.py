...
class Bomberman:
	...
	def explosion(self,position):
		...
		destroyed_cells = []
		for y in range(4):
			if(position[0] + block_size * y < screen_width - 50 and
			[position[0] + block_size * y, position[1]] not in self.blocks):
				pygame.draw.rect(self.screen, (255, 165, 0), [position[0] + block_size * y, position[1], block_size, block_size])
				destroyed_cells.append([position[0] + block_size * y, position[1]])
			else:
				break

		for y in range(4):
			if(position[0] - block_size * y > 0 and 
			[position[0] - block_size * y, position[1]] not in self.blocks):
				pygame.draw.rect(self.screen, (255, 165, 0), [position[0] - block_size * y, position[1], block_size, block_size])
				destroyed_cells.append([position[0] - block_size * y, position[1]])
			else:
		...
		for x in range(4):
			if(position[1] + block_size * x < screen_width - 50 and 
			[position[0], position[1] + block_size * x] not in self.blocks):
				pygame.draw.rect(self.screen, (255, 165, 0), [position[0], position[1] + block_size * x, block_size, block_size])
				destroyed_cells.append([position[0],position[1] + block_size * x])
			else:
				break

		for x in range(4):
			if(position[1] - block_size * x > 0 and
			[position[0], position[1] - block_size * x] not in self.blocks):
				pygame.draw.rect(self.screen, (255, 165, 0), [position[0], position[1] - block_size * x, block_size, block_size])
				destroyed_cells.append([position[0], position[1] - block_size * x])
			else:
	...
	def isOutsideOfMap(self):
		return (self.player.position[0] < 50 or
				self.player.position[1] < 50 or 
				self.player.position[0] >= screen_width - 50 or 
				self.player.position[1] >= screen_height - 50)
		...
		while self.net.current_minigame() == minigameID:
			...
			for event in pygame.event.get():
				if event.type == pygame.QUIT:
					...
					if(self.isOutsideOfMap() or
					self.player.position in self.blocks or
					self.player.position in self.player.bombs or
					self.player.position in self.enemy.bombs or
					self.player.position == self.enemy.position):
